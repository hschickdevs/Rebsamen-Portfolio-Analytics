...
def main() -> None:
    #-------------------------------------HEADER AND TUTORIAL---------------------------------------------------- """
    st.header("Rebsamen Portfolio Overview")
    ...
    # ----------------------------- UPLOAD YOUR CSV FROM FIDELITY SUBHEADER SECTION -------------------------------"""
    st.subheader("Upload the Rebsamen Holdings CSV")
    ...
    if uploaded_data is None:
        ...
        uploaded_data = open(Path('assets/example.csv'), "r")
    else:
    ...
    st.subheader(f'Value of Account{"s" if len(account_selections) > 1 else ""}')
